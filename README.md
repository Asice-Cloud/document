...
将变量通过某些运算符号连接起来，就是表达式了。比如在C语言中，
...
逻辑运算会返回真，假两个值其中之一，判断规则是第二列所示。这里还是使用C语言的运算符演示：
| 符号 | 含义                         |
| ---- | ---------------------------- |
| ==   | 左右两个元素是否相等         |
| !=   | 左右两个是否不相等           |
...
一般情况下，编程语言中都是在 if 关键字语句中使用逻辑运算，也称"控制语句"，例如在C语言中：
...
2.一般情况下，一个{}里的全部内容称作代码块，代码块内部可以定义变量，调用函数等。我们称在某个代码块内部定义的变量为它的局部变量，对应的，在整个代码文件中、在所有{}之外定义的就是全局变量。如果A代码块在B代码块的外部，那么A代码块是不能使用B代码块的局部变量的。（相当于你的钥匙没法打开隔壁宿舍的门，更进一步的，你也可以认为某些实数域上的法则是不能放到复数域的）。
...
在函数式编程中，函数是头等对象，这意味着一个函数，既可以作为其它函数的输入参数值，也可以从函数中返回值，被修改或者被分配给一个变量。λ演算是这种范型最重要的基础，λ演算的函数可以接受函数作为输入参数和输出返回值。这里使用Go语言演示：
...
我们这里先执行了add,后面是subtract，每一个函数都需要时间来执行，当然可以观测函数运行的时间，不同语言中关键字不同罢了，在C++中可以这样做：
...
​	我们希望一边下载/观看东西一边随便输入点内容来评论，常规操作可能是这样的（使用C/C++演示）：
...
#### <span id="build">实践篇其3-从零开始的代码编译原理</span>
...
我们使用的go语言，就需要编译成机器码才能运行。go语言的工具包里提供了编译的工具，可以使用命令`go bulid`来对代码进行编译
...
> 如果没有-o参数，则结果和被编译的文件名相同。如果连文件名都没有，则会对当前目录进行编译。总之，编译之后会生成一个可执行文件，在当前目录下。

当然了，在开发过程中，我们肯定是使用`go run <文件名>`来运行了，它会生成一个临时的可执行文件。
...
你可能暂时不太理解每个细节，没有关系，先通过上面图片的大标题，感受一下编译的过程。
